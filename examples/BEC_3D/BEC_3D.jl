...
# creating a grid
grid = Grid((nx, ny, nz), (xrange, yrange, zrange); array_type=device)
...
param.pot = PotentialQuarticQuadratic(field; γx=0.0, γy=0.0, γz=0.5, κ4=0.01)
solve!(nummodel; istart=niter * 3, plot=false);

using GLMakie
GLMakie.activate!()
ϕc = zeros(grid.nx, grid.ny, grid.nz);
copy!(ϕc, field.ϕ)
surface(ϕc[:, :, grid.nz ÷ 2])
